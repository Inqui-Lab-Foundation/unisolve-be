...
describe("Videos test cases", () => {
    describe("Videos: create the course", () => {
        ...
        test("Should return 200 & create account", async () => {
            ...
            jest.spyOn(videos, "create").mockImplementation(() => mockCreateIntense());
    ...
    describe("Videos: get the single course", () => {
        ...
        test("Should return 200", async () => {
            ...
            jest.spyOn(videoServices, "findVideo").mockImplementation(() => mockCreateIntenseCourse())
            const res = await supertest(app)
    ...
    describe("Videos: update the single video", () => {
        ...
        test("Should return 200", async () => {
            ...
            jest.spyOn(videoServices, "updateVideo").mockImplementation(() => mockCreateIntenseUpdate());
            const res = await supertest(app)
    ...
    describe("Videos: delete the single video", () => {
        ...
        test("Should return 200", async () => {
            ...
            jest.spyOn(videoServices, "findVideo").mockImplementation(() => mockCreateIntenseCourse())
            jest.spyOn(videoServices, "destroyVideo").mockImplementation(() => mockCreateIntenseDelete())
            const res = await supertest(app)
            ...
            expect(mockCreateIntenseCourse).toHaveBeenCalledTimes(1);
            expect(res.status).toBe(200)
            expect(res.body).toHaveProperty("deleteVideo")
        });
    });
});