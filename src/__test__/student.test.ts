...
describe("API - studentChangePassword", () => {
    ...
    test("return 202 change the password", async () => {
        ...
        const res = await request(app).put("/api/v1/student/changePassword").send(changePassword2).set("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..vC0zUP3zUAsOBNp-Hg6Hl-_2hleFAJptyTnBQYRGR9u6VmtZ2OTPgJRKu84T5Eg0wYjt2d6ANFRg7aE4lVhpu4ndmpdFBrEuIDp_dClK6lsFDVrif5QGmu0afHcrR1b6YBmN-_w0C-d__rQQr9WSZZvT40kP22So0nFtUwOGXqY")
        expect(mockCreateIntense).toHaveBeenCalledTimes(1);
        expect(res.statusCode).toBe(202);
        expect(res.body).toHaveProperty("message");
    });
    test("return 400 if the palyload is empty", async () => {
        const res = await request(app).put("/api/v1/student/changePassword").send().set("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..vC0zUP3zUAsOBNp-Hg6Hl-_2hleFAJptyTnBQYRGR9u6VmtZ2OTPgJRKu84T5Eg0wYjt2d6ANFRg7aE4lVhpu4ndmpdFBrEuIDp_dClK6lsFDVrif5QGmu0afHcrR1b6YBmN-_w0C-d__rQQr9WSZZvT40kP22So0nFtUwOGXqY");
        expect(res.statusCode).toBe(400);
    });
    test("return 401 if the token is missing", async () => {
        const res = await request(app).put("/api/v1/student/changePassword").send(changePassword)
        expect(res.statusCode).toBe(401);
    });
