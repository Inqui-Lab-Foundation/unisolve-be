import supertest from "supertest";
import createServer from "../utils/server";
import moduleService from "../services/operational.services";
import { modules } from "../models/modules.model";

const app = createServer();

// test cases for modules API's
describe("modules test cases", () => {
    describe("modules: create the modules", () => {
        const Payload = {
            "course_id": "fg56511234",
            "description": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Inde sermone vario sex illa a Dipylo stadia confecimus. Idem iste, inquam, de voluptate quid sentit? Paulum, cum regem Persem captum adduceret, eodem flumine invectio? Itaque hic ipse iam pridem est reiectus; Duo Reges: constructio interrete. Primum in nostrane potestate est"
        };
        test("Should return 200 & create account", async () => {
            const mockCreateIntense = jest.fn((): any => Payload)
            jest.spyOn(modules, "create").mockImplementation(() => mockCreateIntense());
            const res = await supertest(app).post("/api/v1/modules/create").send(Payload).set("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..vC0zUP3zUAsOBNp-Hg6Hl-_2hleFAJptyTnBQYRGR9u6VmtZ2OTPgJRKu84T5Eg0wYjt2d6ANFRg7aE4lVhpu4ndmpdFBrEuIDp_dClK6lsFDVrif5QGmu0afHcrR1b6YBmN-_w0C-d__rQQr9WSZZvT40kP22So0nFtUwOGXqY")
            expect(mockCreateIntense).toHaveBeenCalledTimes(1);
            // expect(res.body).toHaveProperty("product");
            expect(res.status).toBe(200);
        });
    });
    describe("modules: get the list of the moduless", () => {
        test("Should return 200", async () => {
            const res = await supertest(app)
                .get("/api/v1/modules/list").set("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..vC0zUP3zUAsOBNp-Hg6Hl-_2hleFAJptyTnBQYRGR9u6VmtZ2OTPgJRKu84T5Eg0wYjt2d6ANFRg7aE4lVhpu4ndmpdFBrEuIDp_dClK6lsFDVrif5QGmu0afHcrR1b6YBmN-_w0C-d__rQQr9WSZZvT40kP22So0nFtUwOGXqY")
            expect(res.status).toBe(200)
            expect(res.body).toHaveProperty("products")
        });
    });
    describe("modules: get the single modules", () => {
        const videoPayload = {
            "module": "2",
            "id": "2",
            "status": "Incomplete"
        }
        test("Should return 200", async () => {
            const mockCreateIntensemodules = jest.fn((): any => videoPayload.id);
            jest.spyOn(moduleService, "findOne").mockImplementation(() => mockCreateIntensemodules())
            const res = await supertest(app)
                .get(`/api/v1/modules/get/${2}`).set("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..vC0zUP3zUAsOBNp-Hg6Hl-_2hleFAJptyTnBQYRGR9u6VmtZ2OTPgJRKu84T5Eg0wYjt2d6ANFRg7aE4lVhpu4ndmpdFBrEuIDp_dClK6lsFDVrif5QGmu0afHcrR1b6YBmN-_w0C-d__rQQr9WSZZvT40kP22So0nFtUwOGXqY")
            expect(mockCreateIntensemodules).toHaveBeenCalledTimes(1);
            expect(res.status).toBe(200)
            expect(res.body).toHaveProperty("product")
        });
    });
    describe("modules: update the single modules", () => {
        const updatepayload = { "status": "Completed" }
        const videoPayload = {
            "module": "2",
            "id": "2",
            "status": "Incomplete"
        }
        const payload = [updatepayload, 1];
        test("Should return 200", async () => {
            const mockCreateIntensemodules = jest.fn((): any => videoPayload.id);
            jest.spyOn(moduleService, "findOne").mockImplementation(() => mockCreateIntensemodules())
            const mockCreateIntenseUpdate = jest.fn((): any => { payload[0], payload[1] });
            jest.spyOn(moduleService, "updateOne").mockImplementation(() => mockCreateIntenseUpdate());
            const res = await supertest(app)
                .put(`/api/v1/modules/update/1`).send(updatepayload).set("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..vC0zUP3zUAsOBNp-Hg6Hl-_2hleFAJptyTnBQYRGR9u6VmtZ2OTPgJRKu84T5Eg0wYjt2d6ANFRg7aE4lVhpu4ndmpdFBrEuIDp_dClK6lsFDVrif5QGmu0afHcrR1b6YBmN-_w0C-d__rQQr9WSZZvT40kP22So0nFtUwOGXqY")
            // expect(mockCreateIntenseUpdate).toHaveBeenCalledTimes(1);
            expect(res.status).toBe(200);
            // expect(res.body).toHaveProperty("id")
        });
    });
    describe("modules: delete the single modules", () => {
        const videoPayload = {
            "id": "1",
        }
        test("Should return 200", async () => {
            const mockCreateIntensemodules = jest.fn((): any => videoPayload);
            const mockCreateIntenseDelete = jest.fn((): any => videoPayload);
            jest.spyOn(moduleService, "findOne").mockImplementation(() => mockCreateIntensemodules())
            jest.spyOn(moduleService, "destroyOne").mockImplementation(() => mockCreateIntenseDelete())
            const res = await supertest(app)
                .delete(`/api/v1/modules/delete/${1}`).set("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..vC0zUP3zUAsOBNp-Hg6Hl-_2hleFAJptyTnBQYRGR9u6VmtZ2OTPgJRKu84T5Eg0wYjt2d6ANFRg7aE4lVhpu4ndmpdFBrEuIDp_dClK6lsFDVrif5QGmu0afHcrR1b6YBmN-_w0C-d__rQQr9WSZZvT40kP22So0nFtUwOGXqY")
            expect(mockCreateIntensemodules).toHaveBeenCalledTimes(1);
            expect(res.status).toBe(200)
            expect(res.body).toHaveProperty("deletedCourse")
            expect(res.body).toHaveProperty("text")
        });
    });
});